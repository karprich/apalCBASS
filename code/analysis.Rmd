...
ggplot(data = world) +
 ...
 geom_point(data=siteGpsPoints, aes(x=Lon, y=Lat, fill=Nursery), pch=23, alpha=0.5, show.legend = F, size=3.5)+
...
#save map 
#ggsave("../outputs/siteMap.png", width=10, height=10)
...
meta %>%
  filter(str_detect(Species, "palm")) %$%
  table(Nursery) %>% 
  kable(col.names = c('Nursery', 'Number of Genets'), caption = 'Breakdown of genets by nursery for 172 assessed') %>%
  kable_styling(bootstrap_options = c('striped', 'hover', 'condensed'), full_width = FALSE)
...
fullModelInteraction <- edShDat%>%
  lm(ed50~logSh+nursery:dom, data=.)
...
#set confidence interval to determine
setConfidence <- .99
...
demingNursery <- bind_rows(demingMathMote, demingMathRR, demingMathUM) %>%
  mutate(nursery=factor(nursery, levels = c("UM", "RR", "CRF", "MML")))
```
...
ggplot(demingFigDat, aes(10^logSh, ed50))+
  ...
  geom_text(data=demingEqDf, inherit.aes = F, family="Palatino", size=6,
            aes(x=.4, y=39,label=ifelse(nursery=="CRF", "NS", paste0("Deming Regression:\ny= ",
...
#ggsave("../outputs/demingNurseries.png", width=18, height=14)
...
nurseCow <- domRes %>% 
 ggplot(., aes(x = ed50, fill = nursery)) +
  geom_vline(data=filter(nursStats),  aes(xintercept=mean, color=nursery), size=0.5, show.legend = F, linetype="dashed")+
  ...
  geom_label_repel(family="Palatino", data=filter(nursStats), inherit.aes = F, aes(x=mean, label=group, color=nursery), y=2.5, size=5, show.legend = F, direction="y", label.padding = 0.1, seed = 101, min.segment.length = 100 )
nurseCow
...
shCow <- domRes %>% 
 ggplot(., aes(x = 10^logSh, fill = nursery)) +
  geom_vline(data=filter(shDf),  aes(xintercept=10^emmean, color=nursery), size=0.5, show.legend = F, linetype="dashed")+
  ...
  geom_label_repel(family="Palatino", data=filter(shDf), inherit.aes = F, aes(x=10^emmean, label=.group, color=nursery), y=2.65, size=5, show.legend = F, direction="x", nudge_x = c(0.05, 0, 0.05, 0.05), label.padding = 0.1, seed = 101, min.segment.length = 100 )+
   scale_x_log10(name="S:H",limits=c(0.001, 1), breaks=10^seq(-3, 0, by=0.5), labels=label_math(10^.x, format=log10),
                   guide = "prism_minor",
                minor_breaks=10^seq(-3, 0, by=0.1))+
  scale_y_continuous(limits = c(0,3), guide = "prism_minor", minor_breaks = seq(0,3, 0.5))+
...
demingCow <- ggplot(edShDat, aes(10^logSh, ed50))+
  geom_line(data=demingMath, aes(color=dom, y=fit), size=1.2, show.legend = F)+
  geom_point(aes(fill=nursery, shape=dom), alpha=0.7, size=3, show.legend = c(fill=TRUE, shape=FALSE)) +
  theme_present(18)+
  ...
  guides(fill = guide_legend( 
    override.aes=list(shape = c(21, 23, 21, 21))))
...
ggplot(multi.adj, aes(x = geno, y = ed50_adj, color = nursery, shape = nursery, group = geno)) + 
  ...
  geom_point(aes(y = geno_mean_ed50), pch = 1, color = "black", stroke = 0.25, size = 2.5)+
...
#ggsave(filename = "../outputs/figGenoPairs.png")
...
genoMod <- glm(ed50_adj~geno+nursery, data=multi.adj)
summary(genoMod)
Anova(genoMod)
...
cors%>%
  ...
  facet_wrap(.xkey ~ .ykey, drop=T, nrow=2, labeller =labeller(.xkey=c("RR"="RR vs.", "RR vs. UM", "UM"="UM vs.") )) +
  #facet_manual(facets = vars(.xkey, .ykey), design = "AB
               #C")+
  theme_present(18) +
  geom_smooth(method="glm", se=F)+
  labs(x = "Adjusted ED50 (°C)", y = "Adjusted ED50 (°C)")
#ggsave( filename = "../outputs/nurseryCorrelations.png", width=8)
...
ggplot(., aes(lat, ed50Adj))+
  ...
  stat_cor()+
  labs(x="Longitude", y="ED50(ºC) Adjusted")

#ggsave("../outputs/latitude.png", width=8)
...
#ggsave("../outputs/longitdue.png", width=8)
...
ggplot(., aes(pathfinderV5, ed50Adj))+
  ...
  geom_smooth(method="glm") +
  stat_cor()+
  labs(x="Mean Monthly Maximum (ºC)", y="ED50(ºC) Adjusted")

#ggsave("../outputs/mmm.png", width=8)
