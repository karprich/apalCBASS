...
Use Type I ANOVA (sequential sum of squares, SS) to partition variance in ED50 across:

Log-transformed S:H ratio (logSh). 
Nursery (nursery). 
Dominant symbiont type (dom). 
Genet identity (galaxyCall). 

Since dom is completely confounded with nursery, and therefore statistically inseparable, we fit two models to estimate its potential contribution:  

Model 1 (logSh → Nursery → Dom → Genet) estimates a lower bound for dom’s effect.  
Model 2 (logSh → Dom → Nursery → Genet) estimates an upper bound for dom’s effect.  

```{r}
# Fit Model 1 (logSh → Nursery → Dom → Genet) - Lower Bound for Dom Effect (=zero...)
#### Assigns variance to nursery before dom, so nursery absorbs all the variation
df <- edShDat

model1 <- lm(ed50 ~ logSh + nursery + dom + galaxyCall, data = df)
anova(model1)  # Type I ANOVA (Sequential SS)

# Fit Model 2 (logSh → Dom → Nursery → Genet) - Upper Bound for Dom Effect
#### Assigns variance to dom before nursery, so nursery picks up only variation that can't be explained by dom
model2 <- lm(ed50 ~ logSh + dom + nursery + galaxyCall, data = df)
anova(model2)

# Compute variance explained (SS percentages)
varExpl <- function (mod) {
  setNames(anova(mod)$Sum/sum(anova(mod)$Sum), nm = row.names(anova(mod)))
}

# Extract variance explained for Models 1 and 2
var_explained_model1 <- enframe(varExpl(model1)) %>% mutate(model = "Model 1 (logSh → Nursery → Dom → Genet)")
var_explained_model2 <- enframe(varExpl(model2)) %>% mutate(model = "Model 2 (logSh → Dom → Nursery → Genet)")
var_explained <- bind_rows(var_explained_model1, var_explained_model2) %>%
  complete(name, model, fill = list(value = 0)) %>%
  mutate(name = factor(name, levels = c("logSh", "nursery", "dom", "galaxyCall", "Residuals")))
...
So dom could explain 0% (model 1), if it was actually just a nursery effect.  
Or, dom could explain up to 36.2% (model 2), if it is fully a dom effect.  
So is it more likely to be a nursery effect or a dom effect?  

To determine whether MML’s high ED50 values represent just another nursery effect or something else, we:  

Compute the mean ED50 difference between MML and other nurseries.  
Compare this to the typical ED50 difference between non-MML nurseries.  
Performed a one-sample t-test to test whether MML’s effect is significantly larger than expected based on normal nursery variation.  

```{r}
# Compute all pairwise differences between non-MML nurseries
nurseries <- c("UM", "RR", "CRF")
pairwise_diffs <- c()

for (i in 1:(length(nurseries) - 1)) {
  for (j in (i + 1):length(nurseries)) {
    nursery1 <- nurseries[i]
    nursery2 <- nurseries[j]
    mean_diff <- abs(mean(filter(df, nursery == nursery1) %>% pull(ed50)) - 
                     mean(filter(df, nursery == nursery2) %>% pull(ed50)))
    pairwise_diffs <- c(pairwise_diffs, mean_diff)
  }
}

# Compute mean and standard deviation of typical nursery differences
mean_pairwise_diff <- mean(pairwise_diffs)
std_pairwise_diff <- sd(pairwise_diffs)
n_pairs <- length(pairwise_diffs)  # Number of pairwise comparisons

# Compute observed difference between MML and other nurseries
mml_mean_ed50 <- mean(filter(df, nursery == "MML") %>% pull(ed50))
other_nurseries_mean_ed50 <- mean(filter(df, nursery != "MML") %>% pull(ed50))
observed_diff <- mml_mean_ed50 - other_nurseries_mean_ed50

# Perform one-sample t-test
t_stat <- (observed_diff - mean_pairwise_diff) / (std_pairwise_diff / sqrt(n_pairs))
p_value <- 1 - pt(t_stat, df=n_pairs-1)  # One-tailed test

# Print results
print(paste("Mean pairwise difference between non-MML nurseries:", mean_pairwise_diff))
print(paste("Standard deviation of pairwise differences:", std_pairwise_diff))
print(paste("Observed difference in ED50 (MML vs. other nurseries):", observed_diff))
print(paste("t-statistic:", t_stat))
print(paste("p-value:", p_value))
```

MML’s effect (1.90 ED50 units) is ~3.2× larger than the typical nursery difference (~0.59 ED50 units).  
The one-sample t-test confirmed that MML’s effect is significantly larger than typical nursery variation (p = 0.01698).  
Dom explains between 0% (Model 1) and 36.23% (Model 2) of variance.  
Since MML’s effect is much larger than normal nursery variation, this suggests that either dom or a unique MML environment is responsible. This, plus the fact that higher tolerance is consistent with and exactly what we would expect from Durusdinium, we conclude it is more likely a dom effect than a nursery effect.  

So, we present the variance percentages from Model 2 in the Table:

```{r}
var_explained_model2 %>%
  select(1:2) %>%
  pivot_wider() %>%
  knitr::kable()
```
...
Conclusion
While dom could explain up to 36.23% of variance, it is statistically inseparable from nursery effects. However, MML’s ED50 values are significantly higher than normal nursery variation, strongly suggesting that either dom or a unique MML effect is responsible.
